...
// BenchmarkAdvancedErrorField benchmarks ErrorField creation
func BenchmarkAdvancedErrorField(b *testing.B) {
...
// BenchmarkAdvancedNamedError benchmarks NamedError creation
func BenchmarkAdvancedNamedError(b *testing.B) {
	err := fmt.Errorf("benchmark error: %d", 404)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = NamedError("benchmark_error", err)
	}
}

// BenchmarkAdvancedStringer benchmarks Stringer field creation
func BenchmarkAdvancedStringer(b *testing.B) {
	val := testStringer{value: "benchmark value"}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = Stringer("benchmark", val)
	}
}

// BenchmarkAdvancedObject benchmarks Object field creation
func BenchmarkAdvancedObject(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = Object("benchmark_obj", obj)
	}
}

// BenchmarkAdvancedErrors benchmarks Errors field creation
func BenchmarkAdvancedErrors(b *testing.B) {
	errs := []error{
		errors.New("error 1"),
		errors.New("error 2"),
		fmt.Errorf("error %d", 3),
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = Errors("benchmark_errors", errs)
	}
}
